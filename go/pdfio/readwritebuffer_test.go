...
// TestWriteAtAnExactChunkBoundaryOverwritesTheFirstByte pins the seek-to-end
// arithmetic of RandomAccessReadBuffer.
//
// seek past the end takes the "jump to the end of the buffer" branch, which
// sets the chunk index to the last chunk and the offset within it to
// `size % chunkSize`. Where size is an exact multiple of the chunk size that
// offset is 0 — the *start* of the last chunk, not the end of it. Reading
// recovers, because every read seeks first; writing does not, so a write at the
// end of the buffer lands on the first byte of the last chunk instead.
//
// The buffer is then also unreadable: size counts the write, the chunks do not
// hold it, and a read that walks to the end runs out of chunks.
//
// Read out of the running Java, JDK 17, an 8-byte chunk written full, seeked to
// 8, and written one more byte:
//
//	size=9 position=9
...
//	    buffer reached
//
// See migration/JAVA-BUGS.md entry 69.
func TestWriteAtAnExactChunkBoundaryOverwritesTheFirstByte(t *testing.T) {
	...
	if length != 9 {
		t.Errorf("Length() = %d, want 9: the write counted", length)
	}

	noError(t, "Seek(0)", SeekTo(buffer, 0))
	first := make([]byte, 8)
	n, err := buffer.Read(first)
	noError(t, "Read", err)
	if n != 8 {
		t.Fatalf("Read read %d bytes, want 8", n)
	}
	want := []byte{99, 2, 3, 4, 5, 6, 7, 8}
	for i := range want {
		if first[i] != want[i] {
			t.Errorf("byte %d = %d, want %d — the write at position 8 landed "+
				"on byte 0", i, first[i], want[i])
		}
	}

	// and the ninth byte the length promises is not there
	noError(t, "Seek(0)", SeekTo(buffer, 0))
	if err := ReadFully(buffer, make([]byte, 9)); err == nil {
		t.Error("ReadFully of the whole length reported nothing, want the " +
			"error Java raises: no more chunks available")
	}
}
