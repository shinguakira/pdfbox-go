...
| [`BENCHMARK.md`](BENCHMARK.md) | Speed and memory against PDFBox, both run on one machine: the total, the median, CPU, start-up, what it takes to ship, and where the time goes. **The port's speed and memory are stated here**, with the machine and the method beside them; `PERFORMANCE-PLAN.md` keeps the profiles the performance work started from and `RASTER-PRECEDENT.md` its library comparison |
